// server.js - multi-DB robust uploader (Postgres BYTEA) with safer error handling
...
let poolInfos = []; // array of { name: 'DATABASE_URL1', rawEnvKey: 'DATABASE_URL 1', pool, connString }
  ...
  try {
    ...
    for (const item of normalized) {
      ...
      try {
        ...
        poolInfos.push({ name: normKey, rawEnvKey: rawKey, pool, connString: conn });
...
async function saveFileToDB(token, entry, buffer) {
  ...
  for (const pinfo of poolInfos) {
    try {
      ...
      // If error suggests size/oom, create a special error code and throw (fail-fast)
      ...
      // For other errors (auth/connect), try next pool
      continue;
    }
  }
  // all pools failed
